...
function inferDocType(filename: string): string {
  ...
  if (lower.includes("identity") || lower.includes("passport") || lower.includes("aadhaar") || lower.includes("id")) {
    return "Identity proof";
  }
  if (lower.includes("address") || lower.includes("bill") || lower.includes("statement") || lower.includes("utility")) {
    return "Address proof";
  }
  if (lower.includes("certificate") || lower.includes("degree") || lower.includes("diploma") || lower.includes("education")) {
    return "Education / Birth";
  }
...
export default function DocumentsPage() {
  ...
  const loadDocuments = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to load documents from backend.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filteredDocs = documents.filter((doc) => {
    ...
    return (
      ...
      inferDocType(doc.filename).toLowerCase().includes(q)
    );
  });
          ...
          <SectionTitle
            ...
            action={
              <button
                ...
                className="flex items-center gap-2 rounded-xl border border-[#dddcd6] bg-white px-3 py-2 text-xs font-semibold text-[#5f697b] hover:bg-[#f6f5f2]"
              >
            ...
            {loading ? (
              ...
              <div className="rounded-xl border border-[#f5c6cb] bg-[#fff3f4] p-4 text-xs text-[#c93b4a]">
                ...
                <button
                  ...
                  className="mt-2 rounded-lg bg-[#c93b4a] px-3 py-1 font-semibold text-white hover:bg-[#a82e3b]"
                >
                  Retry
                </button>
              </div>
              ...
              filteredDocs.map((doc) => (
                <DocumentRow
                  ...
                  name={doc.filename}
                  type={inferDocType(doc.filename)}
                  status="Available"
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-[#7f8795]">
                  Documents stay stored locally. The agent uses targeted search and fact extraction to retrieve only the specific evidence required for each rule check.
                </p>
              </div>
            </div>
          </Card>
